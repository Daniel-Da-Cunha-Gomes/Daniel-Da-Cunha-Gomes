...
// une lettre en trop a chaque fois dans le tofind
import (
	"bufio"
	"encoding/json"
	"fmt"
...
func main() { //"rond point du progamme" prépare toutes les valeurs la structure
	...
	if len(os.Args) > 1 {
		if args[0] == "--startWith" {
			a, _ := os.ReadFile("save.txt")
			json.Unmarshal(a, &hangman)
		}
		hangman.Display()
	} else {
		var p [10]string
...
func (h HangManData) Display() { //donne forme au jeu
	...
	for h.Attempts != 0 || h.ToFind == h.Word {
		...
		x := scanner.Text()
		if x == "STOP" || x == "stop" {
			b, err := json.Marshal(HangManData{h.Word, h.ToFind, h.Attempts, h.HangmanPositions})
			if err != nil {
				fmt.Println("Error:", err)
			}
			ioutil.WriteFile("save.txt", b, 1)
			break
		}
		if len(x) == 1 {
			I := IsHere(h.Word, x)
